...
df_cohort_maternity_vaccinated = spark.sql("SELECT * FROM rdp_phi_sandbox.snp2_cohort_maternity_vaccinated_mrna_expanded_7").toPandas().sort_index().fillna(0)
print('# Number of women vaccinated at time of delivery: ' + str(len(df_cohort_maternity_vaccinated)))

df_cohort_maternity_unvaccinated = spark.sql("SELECT * FROM rdp_phi_sandbox.snp2_cohort_maternity_unvaccinated_matched_control").toPandas().sort_index().fillna(0)
print('# Number of unvaccinated pregnant women who are matched controls: ' + str(len(df_cohort_maternity_unvaccinated)))
...
make_histogram(df_cohort_maternity_unvaccinated, 'age_at_start_dt', 1, 10, 60)# run t-test on age at birth for covid-positive vs negative maternity cohorts
...
# evaluate illicit drug use status
...
# evaluate gestational diabetes status
...
# evaluate gestational hypertension status
...
dict_vaccinated_delivery_method.pop(0)
...
# reload saved dataframes
df_cohort_maternity_vaccinated = spark.sql("SELECT * FROM rdp_phi_sandbox.snp2_cohort_maternity_vaccinated_mrna_expanded_7").toPandas().sort_index()
print('# Number of women vaccinated at time of delivery: ' + str(len(df_cohort_maternity_vaccinated)))

df_cohort_maternity_unvaccinated = spark.sql("SELECT * FROM rdp_phi_sandbox.snp2_cohort_maternity_unvaccinated_matched_control").toPandas().sort_index()
print('# Number of unvaccinated pregnant women who are matched controls: ' + str(len(df_cohort_maternity_unvaccinated)))
...
# evaluate RPL Theme 3: Minority Status and Language
...
# print summary statistics on cohort demographics
