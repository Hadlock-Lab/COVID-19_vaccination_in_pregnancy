...
df_cohort_maternity_unvaccinated  = spark.sql("SELECT * FROM rdp_phi_sandbox.snp2_cohort_maternity_vaccinated_but_not_boosted AS mrna_2_shots WHERE mrna_2_shots.ob_delivery_delivery_date >= date_add(mrna_2_shots.full_vaccination_date, 168) AND '2021-09-22' < mrna_2_shots.ob_delivery_delivery_date").dropDuplicates(['pat_id', 'episode_id', 'child_episode_id']).na.drop(subset=['pat_id', 'episode_id', 'child_episode_id'])
...
SELECT v.*
""").toPandas().sort_index().fillna(0)
print('# Number of women vaccinated, but not boosted at time of delivery: ' + str(len(df_cohort_maternity_unvaccinated)))
