...
def format_dataframe_for_psm(df):
  ...
  df = df.drop(columns=['gestational_days', 'insurance', 'race', 'lmp', 'ob_delivery_delivery_date'])
...
print('# Number of unvaccinated pregnant women who have delivered after 1-27-21: ' + str(len(df_control_psm)))
...
# evaluate distribution of time variable between cohorts
print('Overall Stats for Days From Pandemic Start To Conception')
print('# Median Days FromPandemic Start To Conception: ' + str(statistics.median(df_final.pandemic_timing)))
...
print(scipy.stats.pearsonr(df_final_imputed.age_at_start_dt.to_numpy(), df_final_imputed.vaccination_status.to_numpy()))
print('\n')
...
print('\n')
print('Pearson Correlation Days from Pandemic Start to Conception Date:')
...
# declare x and y and scale features
# define
...
# perform variant impact factor analysis
# VIF dataframe
...
# create and evaluate performance of the Logistic Regression Model
...
labels = ['Pandemic Timing', 'Age', 'Commercial Insurance', 'Rural/Urban Categorization', 'Race, Asian', 'Socioeconomic Status', 'Pregravid BMI', 'Housing Density', 'Ethnicity, Hispanic', 'Race, Other', 'Chronic Diabetes', 'Minority Status and Language', 'Race, White', 'Chronic Hypertension', 'Illicit Drug User', 'Race, Black', 'Preterm History', 'Parity', 'Smoker', 'Gravidity', 'Household Composition']
plt.bar([x for x in range(len(importance))], importance, color='goldenrod')
plt.xticks(np.arange(0, 21, 1), labels=labels, rotation = 90)
plt.ylabel('Contribution')
plt.title ('Logistic Regression')
display(plt.show())


# create and evaluate performance of XGBoost Model
...
# plot feature importance
labels = ['Commercial Insurance', 'Rural/Urban Categorization', 'Pandemic Timing', 'Race, Asian', 'Smoker',  'Pregravid BMI', 'Gravidity', 'Ethnicity, Hispanic', 'Age', 'Chronic Diabetes', 'Household Composition', 'Race, Black', 'Race, White', 'Parity', 'Socioeconomic Status', 'Race, Other', 'Illicit Drug User', 'Housing Density', 'Minority Status and Language', 'Preterm History', 'Chronic Hypertension']
plt.bar([x for x in range(len(importance))], importance, color='dimgrey')
plt.xticks(np.arange(0, 21, 1), labels=labels, rotation = 90)
plt.ylabel('Contribution')
plt.title('XGBoost')
display(plt.show())
...
# plot feature importance
labels = ['Pandemic Timing', 'Age', 'Household Composition', 'Housing Density', 'Socioeconomic Status', 'Minority Status and Language', 'Commercial Insurance', 'Pregravid BMI', 'Gravidity', 'Rural/Urban Categorization', 'Parity', 'Race, White', 'Ethnicity, Hispanic', 'Race, Asian', 'Chronic Diabetes', 'Race, Other', 'Illicit Drug User', 'Smoker',  'Preterm History', 'Race, Black', 'Chronic Hypertension']
...
# plot feature importance
labels = ['Pandemic Timing', 'Commercial Insurance', 'Age', 'Household Composition', 'Rural/Urban Categorization', 'Pregravid BMI', 'Race, Asian', 'Socioeconomic Status', 'Housing Density',  'Gravidity', 'Parity', 'Minority Status and Language', 'Ethnicity, Hispanic', 'Smoker', 'Race, Other', 'Chronic Diabetes', 'Race, Black', 'Race, White', 'Preterm History', 'Chronic Hypertension', 'Illicit Drug User']
plt.bar([x for x in range(len(importance))], importance, color='royalblue')
plt.xticks(np.arange(0, 21, 1), labels=labels, rotation = 90)
plt.ylabel('Contribution')
...
# perform variant impact factor analysis for the limited model
X = X[['Age', 'Pandemic Timing', 'Commercial Insurance', 'Pregravid BMI', 'Household Composition']]
...
# create and evalaute performance of the Gradient Boosting Regression Limited Model
# Subset test and training set to only include a limited number of variables
X_train = X_train[['Age', 'Pandemic Timing', 'Commercial Insurance', 'Pregravid BMI', 'Household Composition']]
X_test = X_test[['Age', 'Pandemic Timing', 'Commercial Insurance', 'Pregravid BMI', 'Household Composition']]
...
# plot feature importance
labels = ['Pandemic\nTiming', 'Commercial\nInsurance', 'Age', 'Household\nComposition', 'Pregravid BMI']
plt.figure(figsize=(5, 7))
plt.bar([x for x in range(len(importance))], importance, color='darkorange')
...
plt.plot(fpr_original, tpr_original, 'b', label = 'Original Model AUC = %0.2f' % roc_auc_original, color='royalblue')
plt.plot(fpr, tpr, 'b', label = 'Limited Model AUC = %0.2f' % roc_auc, color='darkorange')
